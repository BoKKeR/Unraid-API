...
export default function startMQTTClient() {
  try {
    // @ts-ignore
    const haOptions = JSON.parse(fs.readFileSync('/data/options.json'))
  ...
  try {
    ...
    client.on('message', async (topic, message) => {
      ...
      let vmIdentifier = ''
      let vmDetails = {} as any
      let vmSanitisedName = ''
      let dockerIdentifier = ''
      let dockerDetails = {} as any
