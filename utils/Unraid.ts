  ...
  Object.keys(servers).forEach((ip) => {
    if (
      ...
      servers[ip].pciDetails =
        servers[ip].vm.details[Object.keys(servers[ip].vm.details)[0]].edit.pcis
    }
    if (!skipSave) {
...
export function flipPCICheck(details, id) {
  ...
  details.pcis
    .filter((pciDevice) => pciDevice.id.split('.')[0] === id.split('.')[0])
    .map((device) => {
      device.checked = check || !device.checked
      if (!check) {
